...
NATIONS_MAP = {
    ...
    "Germany": "Alemanha",
    "Spain": "Espanha",
    "Côte d'Ivoire": "C. Marfim",
    "Cote d'Ivoire": "C. Marfim",
    ...
    "Vanuatu": "Vanuatu",
    "Burundi": "Burundi",
    "North Macedonia": "Macedônia Norte",
    "Bulgaria": "Bulgária",
...
REVERSE_NATIONS_MAP = {v.lower(): k for k, v in NATIONS_MAP.items()}
REVERSE_NATIONS_MAP.update({
    "brasileira": "Brazil",
    "francesa": "France",
    "espanhola": "Spain",
    "inglesa": "England",
...
def translate_nation_pt_to_en(pt_name: str) -> str:
    """Traduz o nome de um país/nacionalidade de PT-BR para EN (Fidelidade FIFA/EA FC)."""
    if not pt_name:
        return ""
    cleaned = pt_name.strip().lower()
    return REVERSE_NATIONS_MAP.get(cleaned, pt_name)

def translate_nation_en_to_pt(en_name: str) -> str:
    """Traduz o nome de um país/nacionalidade de EN para PT-BR (Fidelidade FIFA/EA FC)."""
    if not en_name:
        return ""
    for en_key, pt_val in NATIONS_MAP.items():
        if en_key.lower() == en_name.strip().lower():
            return pt_val
    return en_name
